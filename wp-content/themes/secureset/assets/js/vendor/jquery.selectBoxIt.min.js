...
!function( a ) {
	'use strict'; a( window.jQuery, window, document );
}( function( a, b, c, d ) {
	'use strict'; a.widget( 'selectBox.selectBoxIt', {
		VERSION: '3.8.2',
		options: {
			showEffect: 'none',
			showEffectOptions: {},
		...
		getThemes: function() {
			var b = this,
				c = a( b.element ).attr( 'data-theme' ) || 'c'; return {
				bootstrap: {
					focus: 'active',
					hover: '',
					enabled: 'enabled',
		...
		_createDropdownButton: function() {
			var b = this,
				...
				g = b.selectBox.attr( 'class' ) || ''; return b.dropdownText = a( '<span/>', {
		...
		_replaceSelectBox: function() {
			var b, c, e,
				...
				i = f.listSize = h === d ? 'auto' : '0' === h ? 'auto' : +h; return f.selectBox.css( 'display', 'none' ).after( f.dropdownContainer ), f.dropdownContainer.appendTo( 'body' ).addClass( 'selectboxit-rendering' ), b = f.dropdown.height(), f.downArrow = a( '<i/>', {
		...
		close: function( a ) {
			var b = this,
				...
				f = b.options['native'],
				g = b.isMobile; return f || g || !b.list.is( ':visible' ) || ( b.triggerEvent( 'close' ), 'none' === c ? b.list.hide() : 'hide' === c || 'slideUp' === c || 'fadeOut' === c ? b.list[c]( d ) : b.list.hide( c, e, d ), b.list.promise().done( function() {
		...
		_keydownMethods: function() {
			var a = this,
				b = a.list.is( ':visible' ) || !a.options.keydownOpen; return {
				down: function() {
		...
		_eventHandlers: function() {
			var b, c,
				...
				g = d.focusClass,
				h = d.hoverClass,
				i = d.openClass; return this.dropdown.on({
				...
				'change.selectBoxIt, internal-change.selectBoxIt': function( a, e ) {
					var f, g; e || ( f = d.list.find( 'li[data-val="' + d.originalElem.value + '"]' ), f.length && ( d.listItems.eq( d.currentFocus ).removeClass( d.focusClass ), d.currentFocus = +f.attr( 'data-id' ) ) ), f = d.listItems.eq( d.currentFocus ), g = f.attr( 'data-selectedtext' ), b = f.attr( 'data-text' ), c = b || f.find( 'a' ).text(), d._setText( d.dropdownText, g || c ), d.dropdownText.attr( 'data-val', d.originalElem.value ), f.find( 'i' ).attr( 'class' ) && ( d.dropdownImage.attr( 'class', f.find( 'i' ).attr( 'class' ) ).addClass( 'selectboxit-default-icon' ), d.dropdownImage.attr( 'style', f.find( 'i' ).attr( 'style' ) ) ), d.triggerEvent( 'changed' );
				},
		...
		_update: function( a ) {
			var b, c, d,
				...
				g = e.listItems.eq( e.currentFocus ); 'false' === a.attr( 'data-disabled' ) && ( b = e.listItems.eq( e.currentFocus ).attr( 'data-selectedtext' ), c = g.attr( 'data-text' ), d = c || g.text(), ( f && e.options.html ? e.dropdownText.html() === f : e.dropdownText.text() === f ) && e.selectBox.val() === a.attr( 'data-val' ) ? e.triggerEvent( 'change' ) : ( e.selectBox.val( a.attr( 'data-val' ) ), e.currentFocus = +a.attr( 'data-id' ), e.originalElem.value !== e.dropdownText.attr( 'data-val' ) && e.triggerEvent( 'change' ) ) );
		},
		_addClasses: function( a ) {
			var b = this,
				...
				d = a.list,
				e = a.arrow,
				f = a.container; b.openClass = a.open; return b.selectedClass = 'selectboxit-selected', b.downArrow.addClass( b.selectBox.attr( 'data-downarrow' ) || b.options.downArrowIcon || e ), b.dropdownContainer.addClass( f ), b.dropdown.addClass( c ), b.list.addClass( d ), b;
		},
		...
		triggerEvent: function( a ) {
			var b = this,
				c = b.options.showFirstOption ? b.currentFocus : b.currentFocus - 1 >= 0 ? b.currentFocus : 0; return b.selectBox.trigger( a, {
		...
		var b = this,
			c = a( 'label[for=\'' + b.originalElem.id + '\']' ); return b.dropdownContainer.attr({
			role: 'combobox',
			'aria-haspopup': 'true',
			'aria-expanded': 'false',
		...
		var d = this,
			...
			f = d.options.dontCopyAttributes; return b.length && a.each( b, function( b, d ) {
			var g = d.name.toLowerCase(),
				h = d.value; -1 === a.inArray( g, f ) && ( 'null' === h || -1 === a.inArray( g, e ) && -1 === g.indexOf( 'data' ) || c.attr( g, h ) );
		}), d;
		...
		var b = this; return b.dropdown.off( '.selectBoxIt' ), a.contains( b.dropdownContainer[0], b.originalElem ) && b.dropdownContainer.before( b.selectBox ), b.dropdownContainer.remove(), b.selectBox.removeAttr( 'style' ).attr( 'style', b.selectBoxStyles ), b.triggerEvent( 'destroy' ), b;
	}, e.disable = function( a ) {
		var b = this; return b.options.disabled || ( b.close(), b.selectBox.attr( 'disabled', 'disabled' ), b.dropdown.removeAttr( 'tabindex' ).removeClass( b.theme.enabled ).addClass( b.theme.disabled ), b._setOption( 'disabled', !0 ), b.triggerEvent( 'disable' ) ), b._callbackSupport( a ), b;
		...
		var b = this; return b.originalElem.disabled && b.disable(), b;
	}, e._dynamicPositioning = function() {
		var b = this,
			c = 'selectboxit-open-up',
			d = 'selectboxit-open-down'; if ( 'number' === a.type( b.listSize ) )b.list.css( 'max-height', b.maxHeight || 'none' ); else {
			var e = b.dropdown.offset().top,
				...
				k = i + j >= e + g + f,
				l = !k; if ( b.list.data( 'max-height' ) || b.list.data( 'max-height', b.list.outerHeight() ), l ) if ( b.dropdown.offset().top - j >= f )b.list.css( 'max-height', f ), b.list.css( 'top', b.dropdown.position().top - b.list.outerHeight() ), b.dropdown.addClass( c ); else {
				var m = Math.abs( e + g + f - ( i + j ) ),
					n = Math.abs( b.dropdown.offset().top - j - f ); n > m ? ( b.list.css( 'max-height', f - m - g / 2 ), b.list.css( 'top', 'auto' ), b.dropdown.addClass( d ) ) : ( b.list.css( 'max-height', f - n - g / 2 ), b.list.css( 'top', b.dropdown.position().top - b.list.outerHeight() ), b.dropdown.addClass( c ) );
			} else b.list.css( 'max-height', f ), b.list.css( 'top', 'auto' ), b.dropdown.addClass( d );
		} return b;
	}, e.enable = function( a ) {
		var b = this; return b.options.disabled && ( b.triggerEvent( 'enable' ), b.selectBox.removeAttr( 'disabled' ), b.dropdown.attr( 'tabindex', 0 ).removeClass( b.theme.disabled ).addClass( b.theme.enabled ), b._setOption( 'disabled', !1 ), b._callbackSupport( a ) ), b;
		...
		var b = this; b.currentFocus += 1; var c = 'true' === b.listItems.eq( b.currentFocus ).attr( 'data-disabled' ) ? !0 : !1,
			...
			if ( c && d ) return b.listItems.eq( b.currentFocus - 1 ).blur(), void b.moveDown(); c && !d ? b.currentFocus -= 1 : ( b.listItems.eq( b.currentFocus - 1 ).blur().end().eq( b.currentFocus ).focusin(), b._scrollToView( 'down' ), b.triggerEvent( 'moveDown' ) );
		...
		var b = this; b.currentFocus -= 1; var c = 'true' === b.listItems.eq( b.currentFocus ).attr( 'data-disabled' ) ? !0 : !1,
			...
			if ( c && d ) return b.listItems.eq( b.currentFocus + 1 ).blur(), void b.moveUp(); c && !d ? b.currentFocus += 1 : ( b.listItems.eq( this.currentFocus + 1 ).blur().end().eq( b.currentFocus ).focusin(), b._scrollToView( 'up' ), b.triggerEvent( 'moveUp' ) );
		} return b._callbackSupport( a ), b;
		...
		var d, e, f, g,
			...
			k = h.textArray,
			l = h.currentText,
			m = 'number' === a.type( i.numSearchCharacters ) ? i.numSearchCharacters : 3; for ( d = b, f = k.length; f > d; d += 1 ) {
			for ( g = k[d], e = 0; f > e; e += 1 )-1 !== k[e].search( c ) && ( j = !0, e = f ); if ( j || ( h.currentText = h.currentText.charAt( h.currentText.length - 1 ).replace( /[|()\[{.+*?$\\]/g, '\\$0' ), l = h.currentText ), c = new RegExp( l, 'gi' ), l.length < m ) {
				if ( c = new RegExp( l.charAt( 0 ), 'gi' ), -1 !== g.charAt( 0 ).search( c ) ) return h._setCurrentSearchOption( d ), ( g.substring( 0, l.length ).toLowerCase() !== l.toLowerCase() || h.options.similarSearch ) && ( h.currentIndex += 1 ), !1;
			} else if ( -1 !== g.search( c ) ) return h._setCurrentSearchOption( d ), !1; if ( g.toLowerCase() === h.currentText.toLowerCase() ) return h._setCurrentSearchOption( d ), h.currentText = '', !1;
		} return !0;
		...
		var a, b, c,
			d = this; a = d.selectBox.find( 'option' ).filter( ':selected' ), b = a.attr( 'data-text' ), c = b || a.text(), d._setText( d.dropdownText, c ), d.list.find( 'li[data-val="' + a.val() + '"]' ).find( 'i' ).attr( 'class' ) && d.dropdownImage.attr( 'class', d.list.find( 'li[data-val="' + a.val() + '"]' ).find( 'i' ).attr( 'class' ) ).addClass( 'selectboxit-default-icon' );
	}, e._applyNativeSelect = function() {
		var a = this; return a.dropdownContainer.append( a.selectBox ), a.dropdown.attr( 'tabindex', '-1' ), a.selectBox.css({
			display: 'block',
			visibility: 'visible',
		...
		var b = this; return b.isMobile && ( b._applyNativeSelect(), b._mobileEvents() ), this;
		...
		var d, e,
			...
			h = 0,
			i = ''; if ( 'array' === g ) {
			for ( e = b.length; e - 1 >= h; h += 1 )d = b[h], 'number' === a.type( d ) && ( i += i.length ? ', option:eq(' + d + ')' : 'option:eq(' + d + ')' ); f.selectBox.find( i ).remove();
